...
- **ADD**, **DEL**, **LIST** evaluate the result of JMESPATH as true/false – which determines whether an object is selected for processing. Rules for "falsy" evaluation are as follows: `boolean false, null, empty object, empty list, empty string`
...
- **Get objects where attribute compares against numeric constant**
    ...
    $ ldb add --query 'inference.confidence < `0.8`' 
...
- **Get objects where inference differs from ground truth**
    ...
    $ ldb list --query 'class != inference.class'
    ```
...
- **Get objects where annotation includes certain key**
    ...
    Goal: print objects where annotations have a specific key:
...
- **Get objects with a given number of array members**
...
- **Comparing string against regex**
...
### Features not possible with JMESPATH (need to extend the library or drop these from consideration):

A. Shortcuts for JSON keys regex-style: e.g. `*confidence` as opposed to `class.confidence`

B. Native regex for strings:  e.g.  `class.confidence ≠ 0\.[0-3]*`

C. JMESPATH references grounded in LDB label versions:  `!v1.class.confidence`
